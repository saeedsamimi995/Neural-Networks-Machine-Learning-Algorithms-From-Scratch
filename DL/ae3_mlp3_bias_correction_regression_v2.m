...
for i=1:epochs_ae
    for j=1:n
        ...
        % Back propagation
        f_driviate_d = diag(h2_hat.*(1-h2_hat));  % n1*n1
        f_driviate_e = diag(h3.*(1-h3));  % n2*n2
...
for i=1:max_epoch_p
  for j=1:num_of_train
      ...
      % global train 
      f_driviate_m1=diag(o1.*(1-o1));
      f_driviate_m2=diag(o2.*(1-o2));
      f_driviate_e1 =diag(h1.*(1-h1)); 
      f_driviate_e2 =diag(h2.*(1-h2)); 
      f_driviate_e3 =diag(h3.*(1-h3)); 
      
      w_e1 = w_e1-eta_e*e*-1*1*((w3*f_driviate_m2)*(w2*f_driviate_m1)*(w1*f_driviate_e3)*(w_e3*f_driviate_e2)*(w_e2*f_driviate_e1))'*input; 
      w_e2 = w_e2-eta_e*e*-1*1*((w3*f_driviate_m2)*(w2*f_driviate_m1)*(w1*f_driviate_e3)*(w_e3*f_driviate_e2))'*h1';
      w_e3 = w_e3-eta_e*e*-1*1*((w3*f_driviate_m2)*(w2*f_driviate_m1)*(w1*f_driviate_e3))'*h2'; 
       
      % Back Propagation 
      w1 = w1-eta_p*e*-1*1*((w3*f_driviate_m2)*(w2*f_driviate_m1))'*h3';  
      w2 = w2-eta_p*e*-1*1*(w3*f_driviate_m2)'*o1';
