...
x_n=zeros(n1_neurons,input_num);

%% Encoder 1 Local Train
for i=1:epochs_ae
    for j=1:n1
        % Feed-Forward
        x_n(j,:)=x(j,:)+noise_amp*wgn(1,input_num,noise_var,'linear'); %1*n0
        % Encoder1
        net_e1 = w_e1* x_n(j,:)';  % n1*1
        ...
        delta_w_e1 = (eta_e1 * x_n(j,:)' * err * f_driviate_d * w_d1 * f_driviate_e)';  % n1*n0 = (1*1 * n0*1 * 1*n0 * n0*n0 * n0*n1 * n1*n1)'
...
for i=1:epochs_ae
    for j=1:n1
        ...
        h1_prime=h1+noise_amp*wgn(n1_neurons,1,noise_var,'linear');%n1*1
        % Encoder2
        net_e2 = w_e2* h1_prime;  % n2*1
        ...
        delta_w_e2 = (eta_e2 * h1_prime* err * f_driviate_d * w_d2 * f_driviate_e)';  % n2*n1 = (1*1 * n1*1 * 1*n1 * n1*n1 * n1*n2 * n2*n2)'
...
for i=1:epochs_ae
    for j=1:n1
        ...
        h2_prime=h2+noise_amp*wgn(n2_neurons,1,noise_var,'linear');%n2*1
        % Encoder3
        net_e3 = w_e3* h2_prime;  % n3*1
        ...
        delta_w_e3 = (eta_e3 * h2_prime * err * f_driviate_d * w_d3 * f_driviate_e)';
