...
%% normalizing
data = xlsread('Bias_correction_ucl(noNaN).xlsx');
n = size(data,1);
m = size(data,2);
input_num = m-1;
data_min = min(data);
...
for i=1:epochs_ae
    for j=1:n
        ...
        % Back propagation
        f_driviate_d = diag(x_hat.*(1-x_hat));
        f_driviate_e = diag(h1.*(1-h1));
        
        delta_w_d1 = (eta_e1 * h1 * err * f_driviate_d)';
        delta_w_e1 = (eta_e1 * x(j,:)' * err * f_driviate_d * w_d1 * f_driviate_e)';
...
for i=1:epochs_ae
    for j=1:n
        ...
        % Back propagation
        f_driviate_d = diag(h1_hat.*(1-h1_hat));
        f_driviate_e = diag(h2.*(1-h2));
        
        delta_w_d2 = (eta_e2 * h2 * err * f_driviate_d)';
        delta_w_e2 = (eta_e2 * h1 * err * f_driviate_d * w_d2 * f_driviate_e)';
...
for i=1:epochs_ae
    for j=1:n
        ...
        net_d3 = w_d3 * h3;
        h2_hat = logsig(net_d3);

        % Error
        err = (h2 - h2_hat)';

        % Back propagation
        f_driviate_d = diag(h2_hat.*(1-h2_hat));
        f_driviate_e = diag(h3.*(1-h3));
        delta_w_d3 = (eta_e3 * h3 * err * f_driviate_d)';  
        delta_w_e3 = (eta_e3 * h2 * err * f_driviate_d * w_d3 * f_driviate_e)';
...
w3=unifrnd(lowerb,upperb,[l3_neurons l2_neurons]);
...
for i=1:max_epoch_p
  for j=1:num_of_train
      ...
      f_driviate_e1 =diag(h1.*(1-h1)); 
      f_driviate_e2 =diag(h2.*(1-h2)); 
      f_driviate_e3 =diag(h3.*(1-h3)); 
      
      w_e1=w_e1-eta_e*e*-1*1*((w3*f2_driviate)*(w2*f1_driviate)*(w1*f_driviate_e3)*(w_e3*f_driviate_e2)*(w_e2*f_driviate_e1))'*input; 
      w_e2=w_e2-eta_e*e*-1*1*((w3*f2_driviate)*(w2*f1_driviate)*(w1*f_driviate_e3)*(w_e3*f_driviate_e2))'*h1';
      w_e3=w_e3-eta_e*e*-1*1*((w3*f2_driviate)*(w2*f1_driviate)*(w1*f_driviate_e3))'*h2'; 
       
      % Back Propagation 
      w1=w1-eta_p*e*-1*1*(w3*f2_driviate*w2*f1_driviate)'*h3';  
